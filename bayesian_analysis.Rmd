...
```{r, out.width="100%", fig.width = 8.0, fig.asp = 0.4}
op1 <- est_c %>% 
...
op2 <- est_c %>% 
  ...
  geom_histogram(aes(overall, after_stat(density)), binwidth = 0.001, color = "grey") +
  stat_pointinterval() +
  geom_density(aes(overall)) +
  ...
  geom_vline(xintercept = 0, color = "darkgrey", size = 0.6) +
  xlab("Difference in overall proportion bet") + ylab("") +
  scale_x_continuous(labels = scales::percent_format(1)) +
  ...
  guides(colour = guide_legend("Prior widths"))
cowplot::plot_grid(op1, op2)
```
...
```{r, message=FALSE}
mot_re <- post_both %>% 
  pivot_wider(names_from = parameter, values_from = value, names_repair = "universal") %>% 
