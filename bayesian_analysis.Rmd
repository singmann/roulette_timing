---
title: "Analysis Roulette Speed-Of-Play Study"
...
```{r}
participants_red %>% 
  summarise(across(.cols = age, .fns = c(min = min, 
                                         sd = sd, 
                                         mean = mean, 
                                         max = max,
                                         NAs = ~sum(is.na(.)),
...
```{r}
pgsi %>% 
  summarise(mean = mean(pgsi),
            sd = sd(pgsi),
            perc_larger_7 = mean(pgsi > 7)) 
```
...
pgsi %>% 
  mutate(bin = case_when(
    pgsi == 0 ~ "no risk (0)",
    pgsi <= 2 ~ "low risk (1 - 2)",
    pgsi <= 7 ~ "moderate risk (3 - 7)",
    pgsi > 7 ~ "problem gambler (> 7)",
    TRUE ~ "ERROR"
  )) %>% 
  mutate(bin = factor(bin, levels = c(
    "no risk (0)", "low risk (1 - 2)", "moderate risk (3 - 7)", "problem gambler (> 7)"
  ))) %>% 
  count(bin) %>% 
  mutate(prob = n / sum(n))
```
...
pgsi %>% 
  mutate(bin = case_when(
    pgsi == 0 ~ "recreational (0)",
    pgsi <= 4 ~ "low risk (1 - 4)",
    pgsi <= 7 ~ "moderate risk (5 - 7)",
    pgsi > 7 ~ "problem gambler (> 7)",
    TRUE ~ "ERROR"
  )) %>% 
  mutate(bin = factor(bin, levels = c(
    "recreational (0)", "low risk (1 - 4)", 
    "moderate risk (5 - 7)", "problem gambler (> 7)"
  ))) %>% 
  count(bin) %>% 
  mutate(prob = n / sum(n))
```
...
An additional `r nrow(nib)` betting times (of which `r table(nib$exp_cond)[2]` are in the slowed down version) within the range of 150 to 300 seconds are not shown here.
...
```{r}
part2 %>% 
  summarise(mean = mean(bet_count),
            median = median(bet_count),
            sd = sd(bet_count),
            mean_l0 = mean(bet_count[bet_count != 0]),
            median_l10 = median(bet_count[bet_count != 0]),
            sd_l0 = sd(bet_count[bet_count != 0])) %>% 
  round(1)
```
...
part2 %>% 
  filter(bet_count != 0) %>% 
  group_by(exp_cond) %>% 
  summarise(only1 = mean(bet_count == 1),
            morethan1 = mean(bet_count > 1),
            upto2 = mean(bet_count <= 2),
            only2 = mean(bet_count == 2),
            morethan2 = mean(bet_count > 2))
```
...
bets %>% 
  filter(ppt_id %in% part2$ppt_id) %>% 
  summarise(mean = mean(abs(total_amt_bet)),
            sd = sd(abs(total_amt_bet)),
            n = n())
```

```{r}
select(part2, ppt_id, exp_cond) %>% 
  right_join(bets, by = "ppt_id") %>% 
  filter(ppt_id %in% part2$ppt_id) %>% 
...
```{r}
part2 %>% 
  summarise(mean = mean(average, na.rm = TRUE),
...
```{r, fig.asp=1.0}
plot(mzoib, variable = "_condSlowedMdown", regex = TRUE)
```
...
lpars <- mzoib %>% 
  ...
  mutate(type = case_when(
    ...
    str_detect(.variable, "_condSlowedMdown") ~ "slowed"
  )) %>%  
...
condpars <- lpars %>% 
  ...
  mutate(parameter = factor(
    ...
    levels = c("g", "f", "mu"), 
    labels = par_labels)) %>% 
  mutate(condition = factor(condition, labels = cond_labels))
...
```{r, out.width="100%", fig.width = 9, fig.asp=0.55}
...
```{r, out.width="100%", fig.width = 9, fig.asp=0.4}
...
```{r, out.width="100%", fig.width=6, warning=FALSE, fig.asp=0.4}
...
condpars <- lpars %>% 
  ...
  mutate(parameter = factor(
    ...
    labels = par_labels)) %>% 
  mutate(condition = factor(condition, labels = cond_labels))
#levels(condpars$parameter)

est_c <- condpars %>% 
  pivot_wider(id_cols = c(.chain, .iteration, .draw, condition), 
              names_from = parameter, values_from = estimate) %>% 
  mutate(
    overall = `Gamble at all?` * `Gamble everything?` + 
      `Gamble at all?` * (1 - `Gamble everything?`) * `Proportion bet?`
           )
est_c %>% 
  group_by(condition) %>% 
  mean_qi(overall)
...
est_c %>% 
  compare_levels(overall, condition) %>% 
  mean_qi(overall)
...
```{r}
op1 <- est_c %>% 
  ggplot(aes(x = overall, y = condition)) +
  #stat_histintervalh(breaks = 40) +
  stat_halfeye() +
  xlab("Overall proportion bet") + ylab(ylabel) +
  scale_x_continuous(labels = scales::percent_format(1))
op1
```
...
```{r, fig.asp=1.0}
plot(mzoib2, variable = "_condSlowedMdown", regex = TRUE)
```
...
```{r, fig.width=6, out.width="100%"}
pp_check(mzoib2, type = "hist", binwidth = 0.025, ndraws = 11) +
  coord_cartesian(ylim = c(0, 350))
```
...
lpars <- mzoib2 %>% 
  ...
  mutate(type = case_when(
    str_detect(.variable, "Intercept") ~ "Intercept",
    str_detect(.variable, "condSlowedMdown") ~ "slowed"
  )) %>%  
  mutate(parameter = case_when(
    str_detect(.variable, "zipp") ~ "g",
    str_detect(.variable, "coi") ~ "f",
...
```{r, out.width="100%", fig.width=6, fig.asp=0.45}
comppars <- condpars %>% 
  group_by(parameter) %>% 
  compare_levels(estimate, by = condition) %>% 
  mutate(condition = factor(
...
part2 <- part2 %>% 
  mutate(gamble_cat = if_else(new_prop == 1, "all", 
                              if_else(new_prop == 0, "none", "some"))) %>% 
...
```{r, fig.asp=1.0}
plot(mzoib3, variable = "_condSlowedMdown$", regex = TRUE)
```
...
```{r, fig.width=6, out.width="100%"}
pp_check(mzoib3, type = "hist", binwidth = 0.025, ndraws = 11) +
  coord_cartesian(ylim = c(0, 350))
```
...
pp_check(mnb1, type = "hist", binwidth = 0.025, ndraws = 11, ntrys = 100) 
```
...
pspin1 <- mnb1 %>% 
  ...
  ungroup %>% 
  mutate(exp_cond = factor(exp_cond, labels = cond_labels)) %>% 
...
mnb1 %>% 
  ...
  mutate(mean = exp(.value)) %>% 
  compare_levels(mean, exp_cond) %>% 
  summarise(prop_larger_0 = mean(mean > 0))
```


```{r}
pspin <- cowplot::plot_grid(
  pspin1, pspin2, rel_widths = c(1.5, 1)
  )
ggsave(plot = pspin, 
       filename = "figures/res_spins.png", 
...
```{r, fig.width=6, out.width="100%"}
pp_check(moib, type = "hist", binwidth = 0.025, ndraws = 11) 
```
...
post_both <- left_join(post_moi_mu, post_moi_oi) %>%
  ...
  mutate(parameter = factor(parameter, levels = c("mu", "oi"), 
                            labels = c("Mean bet size", "Bet maximum")),
         exp_cond = factor(exp_cond, labels = cond_labels)) 
pav1 <- ggplot(post_both, aes(x = value, y = exp_cond)) +
  stat_halfeye() +
  facet_wrap("parameter", scales = "free_x") +
  ylab(ylabel) + xlab("Proportion/Probability")  +
  scale_x_continuous(labels = scales::percent_format(1))
pav1
```
...
ggsave(plot = cowplot::plot_grid(pav1, pav2, rel_widths = c(2.25, 1)),
       filename = "figures/res_size.png", 
...
mot_re <- post_both %>% 
  ...
  mutate(pred_mean = (1 - Bet.maximum) * (Mean.bet.size * 2) + Bet.maximum*2) 
...
mot_re_diff <- mot_re %>% 
  compare_levels(pred_mean, exp_cond) 

mot_re_diff %>% 
  median_qi(pred_mean)

mot_re_diff %>% 
  ggplot(aes(pred_mean)) +
  geom_vline(xintercept = 0, color = "grey", size = 0.6) +
  stat_halfeye() +
  labs(x = "Difference in average bet size in £", y = "") +
  theme(axis.ticks.y = element_blank(), 
        axis.text.y = element_blank(), 
        axis.title.y = element_blank(),
        panel.grid.minor.y = element_blank(), 
...
```{r, fig.asp=1.33}
plot(moib2, variables = "Sloweddown|pgsi", regex = TRUE, N = 6)
```
...
```{r, fig.width=6, out.width="100%"}
pp_check(mrisk2, type = "hist", binwidth = 0.05, ndraws = 11)
```
